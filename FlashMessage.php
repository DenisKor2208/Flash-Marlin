...
class FlashMessage {
    private static function put($name, $value) { //записываем значение сессии($value) в ключ с именем $name
    ...
    private static function delete($name) { //удаление записи из сессии
        if(self::exists($name)) {
            unset($_SESSION[$name]);
        }
    }

    private static function get($name) { //получить значение переданного ключа из сессии
